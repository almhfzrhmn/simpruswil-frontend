...
function AdminDashboard() {
  ...
  const getStatsCacheKey = (period, year, month, week) => {
    if (period === "week") return `week_${year}_${week}`;
    if (period === "month") return `month_${year}_${month}`;
  ...
  const processActivityTrendData = useCallback(
    (statsData) => {
      if (!statsData?.trends) {
      ...
      if (selectedPeriod === "week") {
        ...
        if (selectedWeek === null) {
          ...
          for (
            ...
            if (d.getMonth() === currentMonth) {
              // Ensure we don't go to next month
              dailyData.push({
          ...
          bookings.forEach((item) => {
            ...
            if (
              ...
              const dayIndex = Math.floor(
                (date - weekStartDate) / (1000 * 60 * 60 * 24)
              );
        ...
        const targetYear = selectedYear; // Use selected year for month navigation
        ...
        bookings.forEach((item) => {
          ...
          if (selectedPeriod === "year") {
            ...
            shouldInclude = date.getFullYear() === targetYear; // Filter by selected year for year view
          ...
          if (shouldInclude) {
            ...
            const completedCount =
              item.statuses?.find((s) => s.status === "completed")?.count || 0;
            dataMap.get(item._id).completedBookings = completedCount;
          }
        });
        ...
        tours.forEach((item) => {
          ...
          if (shouldInclude) {
            ...
            const completedCount =
              item.statuses?.find((s) => s.status === "completed")?.count || 0;
            dataMap.get(item._id).completedTours = completedCount;
          }
        });
        ...
        let processedData = Array.from(dataMap.values()).sort(
        ...
        const totalBookings = processedData.reduce(
          (sum, item) => sum + (item.bookings || 0),
          0
        );
        const totalTours = processedData.reduce(
          (sum, item) => sum + (item.tours || 0),
          0
        );
        ...
        setPeriodStats({
          ...
          activities: totalBookings + totalTours,
        });
      }
    },
    [selectedPeriod, selectedWeek, selectedMonth, selectedYear, getWeekInMonth]
  );
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const weekNumber =
        selectedPeriod === "week" && selectedWeek !== null
          ? getISOWeek(
              new Date(selectedYear, selectedMonth, 1 + selectedWeek * 7)
            )
          : null;

      // Check if both data are cached
      const statsKey = getStatsCacheKey(
        selectedPeriod,
        selectedYear,
        selectedMonth,
        weekNumber || selectedWeek
      );
      if (dashboardCache && statsCache[statsKey]) {
        setDashboardData(dashboardCache);
        setStatsData(statsCache[statsKey]);
        processActivityTrendData(statsCache[statsKey]);
      ...
      if (!statsCache[statsKey]) {
        promises.push(
          adminAPI.getStats({
            ...
            year: selectedYear,
            ...(selectedPeriod === "month" && { month: selectedMonth }),
            ...(selectedPeriod === "week" &&
              selectedWeek !== null && {
                week: weekNumber,
              }),
      ...
      if (!statsCache[statsKey]) {
        ...
        setStatsData(statsCache[statsKey]);
        processActivityTrendData(statsCache[statsKey]);
        statsRes = { data: { data: statsCache[statsKey] } };
      }
  ...
  useEffect(() => {
    fetchDashboardData();
  }, [fetchDashboardData, selectedWeek, selectedMonth, selectedYear]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-white via-sky-100 to-sky-300 rounded-4xl">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-900 text-lg animate-pulse">
            Memuat Dashboard...
          </p>
        </div>
      </div>
  ...
  const bookingStatusData = [
    {
      name: "Disetujui",
      value: dashboardData.stats.bookings.approved || 0,
      color: "#10b981",
    },
    {
      name: "Menunggu",
      value: dashboardData.stats.bookings.pending || 0,
      color: "#f59e0b",
    },
    {
      name: "Ditolak",
      value: dashboardData.stats.bookings.rejected || 0,
      color: "#ef4444",
    },
    {
      name: "Selesai",
    ...
    {
      name: "Dibatalkan",
      value: dashboardData.stats.bookings.cancelled || 0,
      color: "#6b7280",
    },
  ].filter((item) => item.value > 0); // Only show items with values > 0

  // If no data, show sample data
  const finalBookingStatusData =
  ...
  const todayBookings = (dashboardData?.todayActivities?.bookings || []).filter(
    (item) => item.status === "approved"
  );
  const todayTours = (dashboardData?.todayActivities?.tours || []).filter(
    (item) => item.status === "approved"
  );
  const tomorrowBookings = (
    dashboardData?.tomorrowActivities?.bookings || []
  ).filter((item) => item.status === "approved");
  const tomorrowTours = (dashboardData?.tomorrowActivities?.tours || []).filter(
    (item) => item.status === "approved"
  );
  ...
  const StatCard = ({
    ...
    >
      {/* Glow effect */}
      <div
        className={`absolute inset-0 ${glowColor} opacity-20 group-hover:opacity-40 transition-opacity duration-500 rounded-xl blur-xl`}
      ></div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-sky-100 to-sky-300 p-6">
      ...
      <div className="mb-8 animate-fade-in">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-col gap-3">
            ...
            {/* Year Navigation - Only show when year period is selected */}
            {selectedPeriod === "year" && (
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-600 font-medium">
                  Pilih Tahun:
      ...
      <div className="space-y-8 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <StatCard
            ...
            value={periodStats.users || animatedValues.users}
            change="+12%"
            icon={UserGroupIcon}
          ...
          <StatCard
            ...
            value={
              ...
              (animatedValues.bookings || 0) + (animatedValues.tours || 0)
            }
            change="+11%"
            icon={ChartBarIcon}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <StatCard
            title="Total Booking"
            value={periodStats.bookings || animatedValues.bookings}
            change="+8%"
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 p-4 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 transform transition-all duration-500 hover:scale-105 hover:shadow-2xl group animate-fade-in">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  ...
                  <div className="text-lg font-bold text-gray-900 dark:text-gray-100">
                    {(() => {
                      const approved =
                        dashboardData?.stats?.bookings?.approved || 0;
                      const completed =
                        dashboardData?.stats?.bookings?.completed || 0;
                      const totalApproved = approved + completed;
                      return totalApproved > 0
                        ? Math.min(
                            Math.round((completed / totalApproved) * 100),
                            100
                          )
                        : 0;
                    })()}
              ...
              <div className="space-y-1">
                <h3 className="text-black dark:text-gray-100 text-xs font-medium">
                  Booking
                </h3>
                  ...
                  <div
                    ...
                    style={{
                      width: `${(() => {
                        ...
                        const totalApproved = approved + completed;
                        return totalApproved > 0
                          ? Math.min((completed / totalApproved) * 100, 100)
                          : 0;
                      })()}%`,
                    }}
                  ></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <StatCard
            title="Total Tur"
            value={periodStats.tours || animatedValues.tours}
            change="+15%"
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  ...
                  <div className="text-lg font-bold text-gray-900 dark:text-gray-100">
                    {(() => {
                      const approved =
                        dashboardData?.stats?.tours?.approved || 0;
                      const completed =
                        dashboardData?.stats?.tours?.completed || 0;
                      const totalApproved = approved + completed;
                      return totalApproved > 0
                        ? Math.min(
                            Math.round((completed / totalApproved) * 100),
      ...
      <div
        className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50 shadow-2xl animate-fade-in mb-8"
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="text-xl font-bold text-white">
            Tren Penggunaan Ruangan (Peserta){" "}
            {selectedPeriod === "week"
              ? (() => {
                  const now = new Date();
                  ...
                  if (selectedWeek === null) {
                    ...
                    const startDate = weekStart.toLocaleDateString("id-ID", {
                      day: "numeric",
                    });
                    const endDate = weekEnd.toLocaleDateString("id-ID", {
                      day: "numeric",
                      month: "short",
                    });
                    return `Mingguan - ${startDate} - ${endDate} ${now.getFullYear()} (Minggu ${
                      selectedWeek + 1
                    })`;
                  }
                })()
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div
          className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50 shadow-2xl animate-fade-in"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              {(() => {
                const todayBookings = (
                  dashboardData?.todayActivities?.bookings || []
                ).filter((item) => item.status === "approved");
                const todayTours = (
                  dashboardData?.todayActivities?.tours || []
                ).filter((item) => item.status === "approved");
                return (
                  todayBookings.length + todayTours.length > ITEMS_PER_PAGE && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-slate-300">
                        {todayPage + 1} /{" "}
                        {Math.ceil(
                          (todayBookings.length + todayTours.length) /
                            ITEMS_PER_PAGE
                        )}
                      </span>
                      <button
              ...
              {allTodayActivities
                ...
                .map((activity, index) =>
                  activity.type === "booking" ? (
                    <div
                      key={`today-booking-${
                        todayPage * ITEMS_PER_PAGE + index
                      }`}
                      className="flex items-center gap-4 p-4 bg-green-500/50 rounded-xl border border-green-500/20 hover:bg-green-300/10 transition-colors duration-300"
                    >
                      <div className="p-2 bg-green-500/20 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-gray-800 text-sm">
                          {activity.roomId?.roomName} -{" "}
                          {activity.participantsCount} orang
                        </div>
                      </div>
                      ...
                      <div className="px-3 py-1 bg-green-500/20 text-white text-xs rounded-full">
                        Booking
                      </div>
                    </div>
                  ) : (
                    <div
                      key={`today-tour-${todayPage * ITEMS_PER_PAGE + index}`}
                      className="flex items-center gap-4 p-4 bg-blue-500/50 rounded-xl border border-blue-500/20 hover:bg-blue-300/10 transition-colors duration-300"
                    >
                      <div className="p-2 bg-blue-500/20 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-gray-800 text-sm">
                          {activity.groupName} - {activity.numberOfParticipants}{" "}
                          orang
                        </div>
                      </div>
                      <div className="text-gray-200 text-sm">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-bold text-white">
                  Aktivitas Besok (
                  {tomorrowBookings.length + tomorrowTours.length})
                </h3>
              </div>
              ...
              {(() => {
                ...
                const tomorrowTours = (
                  dashboardData?.tomorrowActivities?.tours || []
                ).filter((item) => item.status === "approved");
                return (
                  tomorrowBookings.length + tomorrowTours.length >
                    ITEMS_PER_PAGE && (
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          setTomorrowPage(Math.max(0, tomorrowPage - 1))
                        }
            ...
            <div className="space-y-4 min-h-[320px]">
              ...
              {allTomorrowActivities
                .slice(
                  tomorrowPage * ITEMS_PER_PAGE,
                  (tomorrowPage + 1) * ITEMS_PER_PAGE
                )
                .map((activity, index) =>
                  activity.type === "booking" ? (
                    <div
                      key={`tomorrow-booking-${
                        tomorrowPage * ITEMS_PER_PAGE + index
                      }`}
                      className="flex items-center gap-4 p-4 bg-orange-500/50 rounded-xl border border-orange-500/20 hover:bg-orange-300/10 transition-colors duration-300"
                    >
                      <div className="p-2 bg-orange-500/20 rounded-lg">
                        <CalendarDaysIcon className="h-5 w-5 text-white " />
                      </div>
                      ...
                      <div className="px-3 py-1 bg-orange-500/20 text-white text-xs rounded-full">
                        Booking
                      </div>
                    </div>
                  ) : (
                    <div
                      ...
                      className="flex items-center gap-4 p-4 bg-purple-500/50 rounded-xl border border-purple-500/20 hover:bg-purple-300/10 transition-colors duration-300"
                    >
                      ...
                      <div className="px-3 py-1 bg-purple-500/20 text-white text-xs rounded-full">
                        Tur
                      </div>
                    </div>
                  )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        >
          ...
          <div className="space-y-4">
            {(() => {
              ...
              const allActivities = [
                ...(dashboardData.recentActivity.bookings || []).map(
                  (item) => ({ ...item, type: "booking" })
                ),
                ...(dashboardData.recentActivity.tours || []).map((item) => ({
                  ...item,
                  type: "tour",
                })),
              ]
              ...
              return allActivities.map((activity, index) => (
                <div
                  ...
                  className="flex items-center gap-4 p-4 bg-slate-700/30 rounded-xl hover:bg-slate-700/50 transition-colors duration-300"
                >
                  <div className="p-2 bg-blue-500/20 rounded-lg">
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-white">
              Permintaan Menunggu (
              {dashboardData.pendingRequests.bookings.length +
                dashboardData.pendingRequests.tours.length}
              )
            </h3>
          </div>
          ...
          <div className="space-y-4">
            {/* Pending Bookings */}
            {dashboardData.pendingRequests.bookings
              .slice(0, 2)
              .map((booking, index) => (
                <div
                  key={`pending-booking-${index}`}
                  className="flex items-center gap-4 p-4 bg-yellow-500/30 rounded-xl border border-yellow-500/20 hover:bg-yellow-500/40"
                >
                  ...
                  <div className="px-3 py-1 bg-yellow-500/20 text-xs rounded-full text-white">
                    Booking
                  </div>
                </div>
              ))}
            ...
            {/* Pending Tours */}
            {dashboardData.pendingRequests.tours
              .slice(0, 2)
              .map((tour, index) => (
                <div
                  key={`pending-tour-${index}`}
                  className="flex items-center gap-4 p-4 bg-orange-500/30 rounded-xl border border-orange-500/20 hover:bg-orange-500/40"
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-gray-800 text-sm">
                      {tour.groupName} - {tour.numberOfParticipants} orang
                    </div>
                  </div>
                  <div className="text-gray-900 text-sm font-semibold">
      ...
      {/* Upcoming Activities */}
      {(dashboardData?.upcomingActivities?.bookings?.length > 0 ||
        dashboardData?.upcomingActivities?.tours?.length > 0) && (
        <div
          className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50 shadow-2xl animate-fade-in mb-6"
        ...
        >
          ...
          <div className="space-y-4">
            {/* Upcoming Bookings */}
            {dashboardData?.upcomingActivities?.bookings?.map(
              (booking, index) => (
                <div
                  key={`upcoming-booking-${index}`}
                  className="flex items-center gap-4 p-4 bg-slate-700/50 rounded-xl border border-indigo-500/20 hover:bg-slate-500 transition-colors-duration-300"
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-gray-800 text-sm">
                      {booking.roomId?.roomName} - {booking.participantsCount}{" "}
                      orang
                    </div>
                  </div>
                  ...
                  <div className="px-3 py-1 bg-indigo-500/20 text-gray-100 text-xs rounded-full">
                    Booking
                  </div>
                </div>
              )
            ...
            {dashboardData?.upcomingActivities?.tours?.map((tour, index) => (
              <div
                key={`upcoming-tour-${index}`}
                className="flex items-center gap-4 p-4 bg-cyan-500/10 rounded-xl border border-cyan-500/20 hover:bg-cyan-500/40 transition-colors-duration-300"
              >
                ...
                <div className="text-gray-800 text-sm font-semibold">
                  ...
                  {new Date(tour.startTime).toLocaleTimeString("id-ID", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </div>
                <div className="px-3 py-1 bg-cyan-500/20 text-gray-100 text-xs rounded-full">
                  Tur
                </div>
              </div>
            ))}
      ...
      {statsData.institutionStats && statsData.institutionStats.length > 0 && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {statsData.institutionStats
              .slice(0, 6)
              .map((institution, index) => (
                <div
                  key={index}
                  className="bg-gradient-to-br from-slate-700/50 to-slate-800/50 p-4 rounded-xl border border-slate-600/50 hover:border-emerald-500/50 transition-all duration-300 transform hover:scale-105"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-slate-400 text-xs">
                          {institution.userCount} pengguna
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="flex justify-between items-center">
                    <div className="text-emerald-400 font-bold">
                      {institution.totalActivity} aktivitas
                    </div>
                    <div className="text-xs text-slate-300">
                      {institution.bookingCount} booking,{" "}
                      {institution.tourCount} tur
                    </div>
                  </div>
                </div>
              ))}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {statsData.popularRooms.slice(0, 6).map((room, index) => (
              <div
                key={index}
                className="bg-gradient-to-br from-slate-700/50 to-slate-800/50 p-4 rounded-xl border border-slate-600/50 hover:border-indigo-500/50 transition-all duration-300 transform hover:scale-105"
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
